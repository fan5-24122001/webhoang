        ...
        <div class="container-fluid">
            <div class="card">
                ...
                <div class="card-body card-block">
                    <form action="{{route('CategoryItem.EditPost',['id'=>$category->id])}}" method="POST" enctype="multipart/form-data" class="form-horizontal">
                    @csrf
                            ...
                            <div class="col-12 col-md-9">
                                <select name="id_category_group" id="">
                                    @foreach ($CategoryGroup as $item)
                                        <option value="{{$item->id}}" {{ ($item->id == $category->id_category_group ) ? 'selected' : ''}} >{{$item->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
